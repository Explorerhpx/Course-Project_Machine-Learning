# applied cross-one-validation on forward stage wise algorithm to select features
import numpy as np
import pickle
from sklearn.model_selection import LeaveOneOut
import feature_selection as fs

Feature_max_size = 96 # the max dimension of features
...
if __name__ == '__main__':
    ...
    # using leave-one-validation to train model
    ...
    for feature_size in range(Feature_max_size): # try different dimension
        print('进行特征维度：',feature_size+1)
        potential_feature = [i for i in range(96) if i not in Feature] # optional features
        ...
        for ite in range(len(potential_feature)):
            ...
            for i in range(len(Train_set)): # cross validation
